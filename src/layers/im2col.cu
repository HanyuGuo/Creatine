...
__global__ void im2col_gpu_kernel(int bs, float *ip,
                 int height,
                 int width,
                 int kernel_sz,
                 int pad,
    ...
    for (int idx = blockIdx.x*blockDim.x+threadIdx.x; i < bs; i+ = gridDim.x*blockDim.x) {
        ...
        for (int i = 0; i < kernel_sz; ++i) {
          for(int j = 0; j<kernel_sz; ++j) {
            ...
            *data_col_ptr = (h_x > 0 && h_y >0 && h_x < width && h_y < height) ? data_im_ptr[i*width+j]:0;
            data_col_ptr += col_height*col_width;
          }
        }
...
void im2col_gpu(const float *ip_img, const int channels, const int height, const int width,
